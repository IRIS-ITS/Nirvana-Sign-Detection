...
int main() {
    const std::string className = "stop";
    const std::string saveDir = "result/" + className;
    
    // Automatically create output directory result/stop if it doesn't exist
    try {
    ...
    // Determine starting file index (stop1.jpg, stop2.jpg, ...)
    int imgIndex = 1;
    ...
    std::cout << "==========================================" << std::endl;
    std::cout << "  Capture Dataset: STOP                   " << std::endl;
    std::cout << "==========================================" << std::endl;
    ...
    while (true) {
        ...
        // Draw HUD UI
        cv::putText(displayFrame, "Class: STOP (640x480)", cv::Point(15, 30),
        ...
        cv::imshow("Dataset Capture - STOP", displayFrame);
        ...
        if (key == 's' || key == 'S') {
            ...
            if (isAutoRecording) {
                savedCountSession = 0;
                std::cout << "[AUTO REC] Started recording STOP frames..." << std::endl;
            } else {
            ...
            std::cout << "Exiting STOP capture program..." << std::endl;
            break;
        }
    }
