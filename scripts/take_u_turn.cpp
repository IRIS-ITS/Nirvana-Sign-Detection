...
int main() {
    const std::string className = "u_turn";
    const std::string saveDir = "result/" + className;
    
    // Automatically create output directory result/u_turn if it doesn't exist
    try {
    ...
    // Determine starting file index (u_turn1.jpg, u_turn2.jpg, ...)
    int imgIndex = 1;
    ...
    std::cout << "==========================================" << std::endl;
    std::cout << "  Capture Dataset: U-TURN                 " << std::endl;
    std::cout << "==========================================" << std::endl;
    ...
    while (true) {
        ...
        // Draw HUD UI
        cv::putText(displayFrame, "Class: U-TURN (640x480)", cv::Point(15, 30),
        ...
        cv::imshow("Dataset Capture - U-TURN", displayFrame);
        ...
        if (key == 's' || key == 'S') {
            ...
            if (isAutoRecording) {
                savedCountSession = 0;
                std::cout << "[AUTO REC] Started recording U-TURN frames..." << std::endl;
            } else {
            ...
            std::cout << "Exiting U-TURN capture program..." << std::endl;
            break;
        }
    }
